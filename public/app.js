...
function renderNavigation(navigation) {
  ...
  navigation.forEach((section, sectionIndex) => {
    ...
    section.items.forEach((item, itemIndex) => {
      ...
      const icon = sectionIndex === 1 ? '▣' : sectionIndex === 0 ? '◦' : '•';
      button.innerHTML = `<span>${icon}</span><span>${item}</span>`;
...
function getMetricWidth(itemIndex, totalItems) {
  const start = 40;
  const step = totalItems > 1 ? 44 / (totalItems - 1) : 0;
  return Math.round(start + itemIndex * step);
}
  ...
  sections.forEach((section) => {
    ...
    section.columns.forEach((column) => {
      ...
      column.items.forEach((item, itemIndex) => {
        ...
        bar.style.width = `${getMetricWidth(itemIndex, column.items.length)}%`;
